...
import (
	...
	"github.com/ollama/ollama/plugin_filtre"
...
type Server struct {
	addr   net.Addr
	sched  *Scheduler
	plugin *plugin_filtre.PluginManager
}
...
// Handler pour gérer le chargemement de la liste.
// Utilise une structure ListRequest pour récupérer les informations nécessaires dans la requête HTTP.
// Vérifie que les informations nécessaires sont dans la requête, et si c'est le cas les utilise pour appeler la fonction LoadList.
func (s *Server) ListLoadHandler(c *gin.Context) {
	...
	if len(req.List) == 0 {
		//gérer erreur possible ou retourne réponse hhtp ok
	}
	//gérer autre erreurs possible par exemple list non existante
	if plugin_filtre.LoadList(req.List, s.plugin) == -1 {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "no list found"})
		return
	}
...
// Handler pour ajouter un mot dans la liste présente dans le PluginManager.
// Récupère les informations via une structure WordRequest puis appelle la fonction
// AddWord().
func (s *Server) ListAddHandler(c *gin.Context) {

	var req api.WordRequest
	if err := c.ShouldBindJSON(&req); errors.Is(err, io.EOF) {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": "missing request body"})
		return
	} else if err != nil {
	...
	if err := plugin_filtre.AddWord(s.plugin, req.Word); err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
// Handler pour supprimer un mot dans la liste présente dans le PluginManager.
// Récupère les informations via une structure WordRequest puis appelle la fonction
// RemoveWord().
func (s *Server) ListRemoveHandler(c *gin.Context) {
	...
	if err := plugin_filtre.RemoveWord(s.plugin, req.Word); err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (s *Server) GenerateRoutes(rc *ollama.Registry) (http.Handler, error) {
	...
	// Plugin
	r.POST("/api/loadlist", s.ListLoadHandler)
	r.POST("/api/addword", s.ListAddHandler)
	r.POST("/api/removeword", s.ListRemoveHandler)
...
func Serve(ln net.Listener) error {
	...
	//MODIFS ICI
	var wordslist = ""
	s.plugin = &plugin_filtre.PluginManager{List: wordslist}
...
func streamResponse(c *gin.Context, ch chan any, s *Server) {
	...
	c.Stream(func(w io.Writer) bool {
		...
		bts = append(bts, '\n')
		str, _ := plugin_filtre.FuzzyLevenshtein(s.plugin, string(bts))
		bts = []byte(str)
...
func (s *Server) ChatHandler(c *gin.Context) {
	...
	if plugin_filtre.TokenizeList(s.plugin, r, c, req.Model) == -1 {
		print("y'a eu un pb")
	}
	...
	if req.Stream != nil && !*req.Stream {
		...
		resp.Message.Content, _ = plugin_filtre.FuzzyLevenshteinText(s.plugin, sb.String())

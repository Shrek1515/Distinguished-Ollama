...
func TestRoutes(t *testing.T) {
	...
	testCases := []testCase{
		{
			Name:   "Load List Handler - valid list",
			Method: http.MethodPost,
			Path:   "/api/loadlist",
			Setup: func(t *testing.T, req *http.Request) {
				reqData := api.ListRequest{Model: "llama3.2", List: "/home/shrek/Documents/L3/ollama/ollama-main/ressources/liste_load_test.txt"}
				body, err := json.Marshal(reqData)
				if err != nil {
					t.Fatalf("failed to marshal ListRequest: %v", err)
				}
		...
		{
			Name:   "Load List Handler - missing list name",
			Method: http.MethodPost,
			Path:   "/api/loadlist",
			Setup: func(t *testing.T, req *http.Request) {
				reqData := api.ListRequest{List: ""}
				body, _ := json.Marshal(reqData)
				req.Body = io.NopCloser(bytes.NewReader(body))
			},
			Expected: func(t *testing.T, resp *http.Response) {
				if resp.StatusCode != http.StatusBadRequest {
					t.Errorf("expected 400 for missing list, got %d", resp.StatusCode)
				}
			},
		},

		{
			Name:   "Add Word Handler - valid word",
			Method: http.MethodPost,
			Path:   "/api/addword",
			Setup: func(t *testing.T, req *http.Request) {
				reqData := api.WordRequest{Word: "foobar"}
				body, _ := json.Marshal(reqData)
				req.Body = io.NopCloser(bytes.NewReader(body))
			},
			Expected: func(t *testing.T, resp *http.Response) {
		...
		{
			Name:   "Remove Word Handler - valid word",
			Method: http.MethodPost,
			Path:   "/api/removeword",
			Setup: func(t *testing.T, req *http.Request) {
				reqData := api.WordRequest{Word: "foobar"}
	...
	s := &Server{}
	s.plugin = &plugin_filtre.PluginManager{
		List: "/home/shrek/Documents/L3/ollama/ollama-main/ressources/liste_remove_test.txt", // tu peux aussi mettre une chaîne vide si le plugin accepte ça
	}
